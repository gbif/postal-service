...
public class DefaultMessagePublisher implements MessagePublisher, Closeable {
  ...
  /**
   * Builds a new MessagingService with the provided components, and declares all the necessary exchanges as topic
   * based.
   *
   * @param connectionParameters to use
   ...
   *
   * @throws IOException if there was a problem declaring an exchange. This might leave the exchanges partially
   *                     declared
   */
  ...
  private void releaseChannel(Channel channel) {
    ...
    if (channel.isOpen()) {
      ...
      LOG.debug("Discarding channel since it is closed. Available channels for reuse: {}", channelPool.size());
    }
  }
